...
function jigo_ce_generate_csv_filename( $export_type = '' ) {

	// Get the filename from WordPress options
	$filename = jigo_ce_get_option( 'export_filename', 'jigo-export_%dataset%-%date%-%random%.csv' );
...
function jigo_ce_generate_csv_header( $export_type = '' ) {

	global $export;

	if( $filename = jigo_ce_generate_csv_filename( $export_type ) ) {
		$mime_type = 'text/csv';
